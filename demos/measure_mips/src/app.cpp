...
#define appconfOUTPUT_NOMINAL_HZ        APP_I2S_AUDIO_SAMPLE_RATE
...
constexpr int decimation_factor = (appconfOUTPUT_NOMINAL_HZ == 32000) ? 3 : 6;
constexpr int stage_2_tap_count = (appconfOUTPUT_NOMINAL_HZ == 32000) ? MIC_ARRAY_32K_STAGE_2_TAP_COUNT : STAGE2_TAP_COUNT;
constexpr const uint32_t* stage_1_filter() {
    return (appconfOUTPUT_NOMINAL_HZ == 32000) ? &stage1_32k_coefs[0] : &stage1_coef[0];
}
constexpr const int32_t* stage_2_filter() {
    return (appconfOUTPUT_NOMINAL_HZ == 32000) ? &stage2_32k_coefs[0] : &stage2_coef[0];
}
constexpr const right_shift_t* stage_2_shift() {
    return (appconfOUTPUT_NOMINAL_HZ == 32000) ? &stage2_32k_shift : &stage2_shr;
}
...
using TMicArray = mic_array::MicArray<mic_count,
                          ...
                          typename std::conditional<DCOE_ENABLED,
...
void app_init()
{
  ...
  unsigned channel_map[MIC_ARRAY_CONFIG_MIC_COUNT] = {2, 3, 4, 5, 6, 7};
